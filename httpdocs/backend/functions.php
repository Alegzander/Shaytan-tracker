 ...
 * Main functions file.
 * @section Modified
 * - $LastChangedDate: 2011-12-14 08:36:17 +0000 (Wed, 14 Dec 2011) $
...
function show_error_msg($title, $message, $wrapper = "1") {
	...
	begin_frame("<font color='#ff0000'>". htmlspecialchars($title) ."</font>");
...
function format_comment($text) {
	...
	$s = preg_replace("/\[img\](http:\/\/[^\s'\"<>]+(\.gif|\.jpg|\.png|\.bmp|\.jpeg))\[\/img\]/i", "<img border='0' src=\"\\1\" alt='' />", $s);

	// [img=http://www/image.gif]
	$s = preg_replace("/\[img=(http:\/\/[^\s'\"<>]+(\.gif|\.jpg|\.png|\.bmp|\.jpeg))\]/i", "<img border='0' src=\"\\1\" alt='' />", $s);
...
function torrenttable($res) {
	...
	foreach ($cols as $col) {
		switch ($col) {
			...
			break;
			case 'uploader':
				echo "<th class='ttable_head'>".T_("UPLOADER")."</th>";
			break;
			case 'comments':
			...
			break;
			case 'health':
				echo "<th class='ttable_head'>".T_("HEALTH")."</th>";
			break;
			case 'external':
				if ($site_config["ALLOWEXTERNAL"])
					echo "<th class='ttable_head'>".T_("L/E")."</th>";
			break;
			case 'added':
				echo "<th class='ttable_head'>".T_("ADDED")."</th>";
			break;
			case 'speed':
				echo "<th class='ttable_head'>".T_("SPEED")."</th>";
			break;
			case 'wait':
				if ($wait)
			...
			break;
			case 'rating':
				echo "<th class='ttable_head'>".T_("RATINGS")."</th>";
			break;
		}
	}
