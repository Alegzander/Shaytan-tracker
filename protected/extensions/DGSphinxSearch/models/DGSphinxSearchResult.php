...
class DGSphinxSearchResult
{
    ...
    private function getIdListByParam(array $list, $param)
    {
        $ret = array();
    ...
    public function getParamIdList($param)
    {
        return $this->getIdListByParam($this->data, $param);
    }
    ...
    public function filterByParamIds($param, array $ids)
    {
        if ($this->enableResultTrace) {
            Yii::trace('Filtered by ' . $param . ' with values: ' . print_r($ids, true), 'CEXT.DGSearch.Result.filterByParamIds');
        }
        if (count($ids) == 0) {
    ...
    {
        return $this->getCriteria()->paginator;
    }
