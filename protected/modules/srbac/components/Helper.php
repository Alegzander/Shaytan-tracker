...
//      $len = strlen($cleverName);
	...
	public static function compilateModelErrorsToString(CModel $model, $template = "{field}: {errors}.\n"){
		$result = '';

		foreach ($model->getErrors() as $field => $errors){
			$errorsString = '"'.implode('", "', $errors).'"';

			$result .= \Yii::t(User::model()->pwgen(16), $template, array(
				'{field}' => $field,
				'{errors}' => $errorsString
			));
	...
	public static function compilateModelAttributesToString(CModel $model){
		$fieldsStringArray = array();

		foreach ($model->attributes as $fieldName => $value){
			array_push($fieldsStringArray, \Yii::t(User::model()->pwgen(16), '{field_name} = ({data_type}) "{value}"', array(
				'{field_name}' => $fieldName,
				'{data_type}' => gettype($model->{$fieldName}),
				'{value}' => $value
			)));
		}
